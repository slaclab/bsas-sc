...
protected:
  ...
  Runnable(const char *name, std::shared_ptr<pvxs::MPMCFIFO<Runnable *>> dead)
      : dead_(dead), running_(false),
        thread_(*this, name, epicsThreadGetStackSize(epicsThreadStackMedium),
...
public:
  Listener(std::shared_ptr<pvxs::MPMCFIFO<Runnable *>> dead,
           const std::vector<std::string> &pvlist,
    ...
    for (auto pvname : pvlist) {
      try {
        ...
        log_err_printf(LISTENER_LOG, "Failed to subscribe to '%s': %s\n",
                       pvname.c_str(), e.what());
      } catch (...) {
    ...
    if (subscriptions_.empty()) {
      throw std::runtime_error(
          "Listener failed to subscribe to any PV from --pvlist");
    }
  }
  ...
  void run() {
    ...
    while (running_) {
      ...
      try {
        ...
        log_warn_printf(LISTENER_LOG,
                        "Can't push new data for '%s', cancelling sub\n",
                        sub->name().c_str());
        sub->cancel();
...
public:
  Reactor(std::shared_ptr<pvxs::MPMCFIFO<Runnable *>> dead,
          const std::shared_ptr<TimeAlignedTable> &taligned_table,
          double period, double timeout, pvxs::server::SharedPV &pv)
      : Runnable(typeid(Reactor).name(), dead), taligned_table_(taligned_table),
        period_(period), timeout_(timeout), pv_(pv) {
    assert(period > 0.0);
  ...
  bool prepare(double sleepPeriod) {
    ...
    log_debug_printf(REACTOR_LOG,
                     "Waiting until all PVs have at least one update%s\n", "");

    while (running_ &&
           (timeout_ == 0 ||
            epicsTimeDiffInSeconds(&now_ts, &start_ts) < timeout_) &&
           !taligned_table_->initialized()) {
  ...
  virtual void run() {
    ...
    while (running_) {
      ...
      double secs_since_last_update =
          epicsTimeDiffInSeconds(&now, &last_update);

      if (timeout_ > 0 && secs_since_last_update > timeout_) {
        log_err_printf(REACTOR_LOG,
                       "Timed out waiting for updates. Waited for %.1f sec "
                       "(timeout=%.1f sec)\n",
      ...
      log_debug_printf(REACTOR_LOG,
                       "Considering timespans shortest=%.6f s, longest=%.6f\n",
      ...
      log_debug_printf(
          REACTOR_LOG,
          "Extracting merged table spanning %.3f sec: %u.%u -- %u.%u\n",
          epicsTimeDiffInSeconds(&end.ts, &start.ts), start.ts.secPastEpoch,
          start.ts.nsec, end.ts.secPastEpoch, end.ts.nsec);
...
int main(int argc, char *argv[]) {
  ...
  auto cli = (clipp::required("--pvlist")
                      .doc("File with list of input NTTable PVs to be merged "
                           "(newline-separated).") &
              ...
              clipp::required("--pvname").doc("Name of the output PV.") &
                  clipp::value("pvname", pvname),

              clipp::option("--label-sep")
                      .doc(std::string("Separator between PV name and column "
                                       "name in labels. Default: '") +
                           label_sep + "'.") &
                  clipp::value("label_sep", label_sep),

              clipp::option("--column-sep")
                      .doc(std::string("Separator between PV identifier and "
                                       "original column name. Default: '") +
                           col_sep + "'.") &
...
#define VALIDATE_ARG(COND, FMT, ARG)                                           \
  ...
  VALIDATE_ARG(timeout_sec < 0.0 ||
                   (timeout_sec > 0 && timeout_sec < period_sec),
               "Invalid timeout: %.6f seconds\n", timeout_sec);
#undef VALIDATE_ARG
  ...
  try {
    ...
    auto taligned_table(
        std::make_shared<TimeAlignedTable>(pvlist, label_sep, col_sep));
