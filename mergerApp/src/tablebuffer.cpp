...
TimeSpan::TimeSpan(const TimeStamp &start, const TimeStamp &end)
  ...
  if (start > end) {
    // char message[1024];
    epicsSnprintf(message, sizeof(message),
                  "TimeSpan expected to have start=%u.%09u.%016lX before "
                  "end=%u%.09u.%016lX",
                  start.ts.secPastEpoch, start.ts.nsec, start.utag,
                  end.ts.secPastEpoch, end.ts.nsec, end.utag);
...
bool TimeStamp::operator<(const TimeStamp &rhs) const {
  return epicsTimeLessThan(&this->ts, &rhs.ts) ||
         (epicsTimeEqual(&this->ts, &rhs.ts) && this->utag < rhs.utag);
}
...
void TableBuffer::update_timestamps() {
  ...
  const auto start_seconds_past_epoch =
      first.get_column_as<const TimeTable::SECONDS_PAST_EPOCH_T>(
          TimeTable::SECONDS_PAST_EPOCH_COL);

  const auto start_nanoseconds =
      first.get_column_as<const TimeTable::NANOSECONDS_T>(
          TimeTable::NANOSECONDS_COL);

  start_ts_ = {start_seconds_past_epoch[inner_idx_],
               start_nanoseconds[inner_idx_]};

  const auto &last = buffer_.back();

  const auto end_seconds_past_epoch =
      last.get_column_as<const TimeTable::SECONDS_PAST_EPOCH_T>(
          TimeTable::SECONDS_PAST_EPOCH_COL);

  const auto &end_nanoseconds =
      last.get_column_as<const TimeTable::NANOSECONDS_T>(
          TimeTable::NANOSECONDS_COL);
...
bool TableBuffer::empty() const { return buffer_.empty(); }
...
TableBuffer::allocate_containers(size_t num_rows) const {
...
std::pair<size_t, size_t> TableBuffer::consume_each_row_inner(ConsumeFunc f) {
  ...
  for (auto v : buffer_) {
    auto seconds_past_epoch =
        v.get_column_as<const TimeTable::SECONDS_PAST_EPOCH_T>(
            TimeTable::SECONDS_PAST_EPOCH_COL);

    auto nanoseconds = v.get_column_as<const TimeTable::NANOSECONDS_T>(
        TimeTable::NANOSECONDS_COL);
    ...
    for (; inner_idx < n; ++inner_idx) {
      ...
      TimeStamp ts{{seconds_past_epoch[inner_idx], nanoseconds[inner_idx]},
                   pulse_id[inner_idx]};
...
size_t
TableBuffer::extract_timestamps_between(const TimeStamp &start,
                                        const TimeStamp &end,
  ...
  for (const auto &buf : buffer_) {
    auto seconds = buf.get_column_as<const TimeTable::SECONDS_PAST_EPOCH_T>(
        TimeTable::SECONDS_PAST_EPOCH_COL);
    auto nanoseconds = buf.get_column_as<const TimeTable::NANOSECONDS_T>(
        TimeTable::NANOSECONDS_COL);
    auto utag =
        buf.get_column_as<const TimeTable::PULSE_ID_T>(TimeTable::PULSE_ID_COL);

    size_t nrows =
        std::min(std::min(seconds.size(), nanoseconds.size()), utag.size());
