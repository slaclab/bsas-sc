...
enum StopReason { INTERRUPTED, TIMEOUT, DISCONNECTED, ERROR };

static const char *STOP_REASON_STR[] = {
    "The program was interrupted", "Timed out while waiting for PV updates",
    "The PV disconnected", "An unexpected error occurred"};
...
static std::string create_folder_and_file(const std::string &base_directory,
  ...
  epicsSnprintf(output_file, sizeof(output_file),
                ...
                base_directory.c_str(), year, month, day, file_prefix.c_str(),
  ...
  for (int component : components) {
    ...
    if (mkdir(out_dir.c_str(), 0777) < 0 && errno != EEXIST)
      throw std::runtime_error(std::string("Failed to mkdir ") + out_dir +
                               ": " + strerror(errno));
  }
...
int main(int argc, char *argv[]) {
  ...
  auto cli =
      (clipp::required("--input-pv").doc("Name of the input PV") &
       ...
       clipp::required("--file-prefix").doc("Prefix for generated HDF5 files") &
       ...
       clipp::required("--timeout-sec")
               .doc(
                   "If no updates are received within timeout (in seconds), "
                   "close the file and exit. A value of 0 means wait forever") &
           clipp::value("timeout_sec", timeout_sec),

       clipp::option("--max-duration-sec")
               .doc("Maximum time, in seconds, to collect data for in a single "
                    "HDF5 file. If 0, don't limit files by time. Default: 0") &
           clipp::value("max_duration_sec", max_duration_sec),

       clipp::option("--max-size-mb")
               .doc("Maximum size, in MB, to collect data for in a single HDF5 "
                    "file. If 0, don't limit files by size. Default: 0") &
           clipp::value("max_size_mb", max_size_mb),

       clipp::option("--label-sep")
               .doc(std::string("separator between PV name and column name in "
                                "labels. Default: '") +
                    label_sep + "'") &
           clipp::value("label_sep", label_sep),

       clipp::option("--column-sep")
               .doc(std::string("separator between PV identifier and original "
                                "column name. Default: '") +
                    col_sep + "'") &
...
#undef CHECK_ARG
  ...
  log_info_printf(LOG, "  timeout=%f s%s\n", timeout_sec,
                  timeout_sec == 0.0 ? " (wait forever)" : "");
  log_info_printf(LOG, "  max duration=%f s%s\n", max_duration_sec,
                  max_duration_sec == 0.0 ? " (no time limit)" : "");
  log_info_printf(LOG, "  max size=%lu MB%s\n", max_size_mb,
                  max_size_mb == 0 ? " (no size limit)" : "");
  ...
  auto subscription =
      client
          .monitor(input_pv)
          // Work around older pvxs monitor parser which expects
    ...
    while (!done) {
      ...
      for (;;) {
        ...
        double wait_for_sec = std::min(
            timeout_sec, max_duration_sec > 0 ? max_duration_sec - elapsed_sec
                                              : timeout_sec);
        ...
        if (!event.wait(wait_for_sec)) {
          if (elapsed_sec > timeout_sec) {
            ...
            // A file is opened and we reached the maximum duration, exit the
            // inner loop so a new file can be generated
            log_info_printf(LOG,
                            "File %s has duration of %.0f sec, which meets or "
                            "exceeds maximum duration of %.0f sec\n",
                            writer->get_file_path().c_str(), elapsed_sec,
                            max_duration_sec);
            break;
        ...
        try {
          while (auto v = subscription->pop()) {
            ...
            if (!writer) {
              epicsTimeGetCurrent(&start); // reset start time so the file has a
                                           // consistent duration
              std::string output_file =
                  create_folder_and_file(base_directory, file_prefix, start);
              writer.reset(new tabulator::Writer(
                  input_pv, output_file, root_group, label_sep, col_sep));
            }
        ...
        if (stat(writer->get_file_path().c_str(), &s) < 0)
          throw std::runtime_error(std::string("Failed to stat output file ") +
                                   writer->get_file_path());

        size_t file_size_mb = s.st_size / 1024 / 1024;

        if (file_size_mb >= max_size_mb) {
          // We reached the maximum file size, exit the inner loop so a new file
          // can be generated
          log_info_printf(LOG,
                          "File %s has size %lu MB, which meets or exceeds "
                          "maximum size of %lu MB\n",
                          writer->get_file_path().c_str(), file_size_mb,
                          max_size_mb);
          break;
        }
      }
  ...
  log_printf(LOG, is_err(stop_reason) ? pvxs::Level::Err : pvxs::Level::Info,
